...
if did:
    ...
    if tid:
        ...
        r = GET(f"/domains/{did}/topics")
        check("GET topics (验证改名)", r, f"专题改名_{uid}")
...
if tid:
    ...
    if cid:
        ...
        r = PATCH(f"/conversations/{cid}", {"name": f"会话改名_{uid}"})
        http_check("PATCH /conversations/{cid}", r, 200)
        
        r = GET(f"/topics/{tid}/conversations")
        check("GET convs (验证改名)", r, f"会话改名_{uid}")
...
if cid:
    ...
    try:
        ...
        if files:
            # Need the FileRecord id, get it from storage
            fname = files[0].get("name", "")
            # Try to find file record id
