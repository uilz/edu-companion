"""Phase 9 测试：Phase8Classifier 文本降级分类"""

import pytest


class TestPhase8Classifier:
    """Phase8Classifier 的文本降级分类测试"""

    def test_keyword_score_math(self):
        """高等数学关键词应该匹配到 '高等数学' 分区"""
        from app.services.phase8_classifier import phase8_classifier

        result = phase8_classifier._keyword_score("导数的定义是什么")
        assert len(result) >= 1
    ...
    def test_keyword_score_multi_subject(self):
        ...
        result = phase8_classifier._keyword_score("矩阵的特征值和概率分布")
    ...
    def test_keyword_score_empty(self):
        ...
        result = phase8_classifier._keyword_score("你好，今天天气不错")
        assert result == []
    ...
    def test_keyword_score_programming(self):
        ...
        result = phase8_classifier._keyword_score("Python 链表反转算法")
        assert len(result) >= 1
    ...
    def test_decide_mode_no_candidates(self):
        ...
        result = phase8_classifier._decide_mode([], None)
    ...
    def test_decide_mode_single_new_topic(self):
        ...
        result = phase8_classifier._decide_mode(candidates, "t0")
    ...
    def test_decide_mode_same_topic(self):
        ...
        result = phase8_classifier._decide_mode(candidates, "t1")
        assert result["mode"] == 3
        assert result["should_switch"] == False
    ...
    def test_decide_mode_multi_close(self):
        ...
        # 0.72 * 1.5 > 0.85 => 接近
        result = phase8_classifier._decide_mode(candidates, None)
    ...
    def test_classify_by_text_empty_text(self):
        ...
        result = phase8_classifier.classify_by_text("test_user", "", None)
        assert result["mode"] == 3
    ...
    def test_classify_fallback_without_embedding(self):
        ...
        result = phase8_classifier.classify("test_user", [], None)
