...
def _ensure_tables():
    ...
    for p in possible_paths:
        if os.path.exists(p):
            ...
            break
    if not sql_path:
        logger.error("找不到建表 SQL 文件 (searched: %s)", possible_paths)
        return
...
def resolve_bank_for_conversation(conversation_id, user_id=DEFAULT_USER_ID, user_specified_bank_id=None):
    ...
    from app.db.database import get_db
    db = get_db()
    conv = db.fetchone("SELECT partition_id, parent_id FROM conversations WHERE id = %s", (conversation_id,))
    if conv and conv.get("parent_id"):
        bank_id = f"bnk_{conv['parent_id']}"
        _ensure_bank(db, bank_id, user_id, conv["parent_id"])
        return bank_id
    if conv and conv.get("partition_id"):
        bank_id = f"bnk_{conv['partition_id']}"
        _ensure_bank(db, bank_id, user_id, conv["partition_id"])
        return bank_id
