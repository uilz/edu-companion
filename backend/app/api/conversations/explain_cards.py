...
- 可拖动（posX / posY）
- 可递归折叠/删除
- D14: 卡片数据存入 messages.metadata.explain_cards JSONB 数组

统一前缀: /api/conversations/explain-cards
"""
...
router = APIRouter(prefix="/api/conversations/explain-cards", tags=["解释卡片"])
...
async def list_cards(
    ...
    from app.services.conversation.message_repository import get_message_repo
    repo = get_message_repo()
    all_messages = repo.load_by_directory(user_id, conv_id)
...
async def create_card(
    ...
    card_id = f"explain_{datetime.now().strftime('%Y%m%d%H%M%S%f')}_{user_id}"
    ...
    cards = _get_cards_from_msg(message_id)
    cards.append(card)
    _save_cards_to_msg(message_id, user_id, cards)
...
async def update_card(
    ...
    _save_cards_to_msg(found_msg_id, user_id, found_cards)
...
async def delete_card(
    ...
    _save_cards_to_msg(found_msg_id, user_id, remaining)
