"""
Knowledge Graph AI — AI 生成 / 扩充 / 编辑 / 对话

统一前缀: /api/knowledge-graph/ai
"""
...
router = APIRouter(prefix="/api/knowledge-graph/ai", tags=["认知图谱 AI"])
...
async def ai_expand_node(
    ...
    from app.services.knowledge_tree.ai_expansion_service import expand_node

    result = await expand_node(user_id, node_id, body.depth, body.direction)
    if not result.get("ok"):
        if result.get("error") == "节点不存在":
            raise HTTPException(404, "知识点不存在")
        raise HTTPException(400, result.get("error", "AI 扩充失败"))
    return result
