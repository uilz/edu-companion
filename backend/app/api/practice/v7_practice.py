...
async def api_unfinished_sessions(user_id: str = Depends(current_user_id)):
    """获取未完成的会话列表（created / active / paused）"""
    _ensure_tables()
    from app.db.database import get_db
    db = get_db()
    rows = db.fetchall(
        """SELECT id, bank_id, session_type, mode, status, total_count,
                  (SELECT COUNT(*) FROM v7_session_questions sq 
                   WHERE sq.session_id = v7_practice_sessions.id AND sq.user_answer IS NOT NULL
                  ) as answered_count,
                  created_at
...
async def api_cancel_session(session_id: str, user_id: str = Depends(current_user_id)):
    """取消会话"""
    _ensure_tables()
...
async def api_exam_submit_all(session_id: str, user_id: str = Depends(current_user_id)):
    """提交考试所有答案，生成成绩报告"""
    _ensure_tables()
...
async def api_import_preview(body: dict, user_id: str = Depends(current_user_id)):
    ...
    from app.services.practice.practice_import import parse_questions_from_text, ai_correct_question, match_cognitive_nodes
...
async def api_answer_history(
    ...
    total = db.fetchone(
        f"SELECT COUNT(*) as cnt FROM v7_practice_attempts a WHERE {where}",
        tuple(params),
    )
    ...
    rows = db.fetchall(
        f"""SELECT a.id, a.session_id, a.question_id, a.user_answer,
            ...
            FROM v7_practice_attempts a
            LEFT JOIN v7_questions q ON a.question_id = q.id
            WHERE {where}
