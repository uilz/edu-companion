...
class AdaptivePlanGenerator:
    ...
    def _find_entry_skills(self, nodes, user_id: str, subject=None):
        """找到所有前置条件已满足的入口技能（使用 checker 的前置数据）"""
        node_map = {n.id: n for n in nodes}
        entry = []
        for sid, prereqs in self._checker._prerequisites.items():
            if subject and (self._checker._subject_map.get(sid) or SKILL_TO_SUBJECT.get(sid)) != subject:
                continue
