...
# 复用主应用的认证中间件（带 role 注入）
app.add_middleware(AdminAuthMiddleware)

# 挂载 4 个模块路由
app.include_router(users.router, prefix="/api/admin/users", tags=["用户管理"])
app.include_router(data.router, prefix="/api/admin/data", tags=["全局数据"])
app.include_router(monitor.router, prefix="/api/admin/monitor", tags=["系统监控"])
app.include_router(analytics.router, prefix="/api/admin/analytics", tags=["BI 分析"])
