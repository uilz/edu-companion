...
export const knowledgeNodesApi = {
  list: (params?: { parent_id?: string; level?: string; search?: string }) => {
    ...
    return api<ListResponse<KnowledgeNode>>(
      `/api/knowledge-graph/nodes${qs ? `?${qs}` : ""}`
    );
  },

  get: (nodeId: string) =>
    api<{ node: KnowledgeNode }>(`/api/knowledge-graph/nodes/${nodeId}`),

  getSubtree: (nodeId: string) =>
    api<{ nodes: Record<string, KnowledgeNode> }>(
      `/api/knowledge-graph/nodes/${nodeId}/subtree`
    ),

  getConversations: (nodeId: string) =>
    api<{ conversations: Conversation[]; total: number }>(
      `/api/knowledge-graph/nodes/${nodeId}/conversations`
    ),
  ...
  create: (data: {
    ...
    api<{ node: KnowledgeNode }>("/api/knowledge-graph/nodes", {
      method: "POST",
  ...
  delete: (nodeId: string) =>
    api<{ ok: boolean }>(`/api/knowledge-graph/nodes/${nodeId}`, {
      method: "DELETE",
    }),

  addPrerequisite: (nodeId: string, prereqId: string, prereqType = "strict") =>
    api<{ ok: boolean }>(
      `/api/knowledge-graph/nodes/${nodeId}/prerequisites`,
      {
        method: "POST",
  ...
  removePrerequisite: (nodeId: string, prereqId: string) =>
    api<{ ok: boolean }>(
      `/api/knowledge-graph/nodes/${nodeId}/prerequisites/${prereqId}`,
      { method: "DELETE" }
    ),
  ...
  addAssociate: (
    ...
    api<{ ok: boolean }>(
      `/api/knowledge-graph/nodes/${nodeId}/associates`,
      {
        method: "POST",
        body: JSON.stringify({
          target_id: targetId,
  ...
  reorderChildren: (nodeId: string, childrenOrder: string[]) =>
    api<{ ok: boolean }>(
      `/api/knowledge-graph/nodes/${nodeId}/reorder`,
      {
        method: "PUT",
...
export const conversationsApi = {
  list: (knowledgeNodeId?: string) => {
    ...
    return api<ListResponse<Conversation>>(
      `/api/knowledge-graph/conversations${params}`
    );
  },

  get: (convId: string) =>
    api<{ conversation: Conversation }>(
      `/api/knowledge-graph/conversations/${convId}`
    ),
  ...
  create: (data?: {
    ...
    api<{ conversation: Conversation }>(
      "/api/knowledge-graph/conversations",
      {
        method: "POST",
        body: JSON.stringify(data || {}),
      }
  ...
  addKnowledgeNode: (convId: string, nodeId: string) =>
    api<{ ok: boolean }>(
      `/api/knowledge-graph/conversations/${convId}/knowledge-nodes/${nodeId}`,
      { method: "POST" }
    ),
...
export const navigationApi = {
  getTree: (rootId?: string) => {
    ...
    return api<{ tree: NavigationTree[] }>(
      `/api/knowledge-graph/navigation${params}`
    );
  },

  getNode: (nodeId: string) =>
    api<{ node: NavigationNode }>(
      `/api/knowledge-graph/navigation/${nodeId}`
    ),

  getChildren: (nodeId: string) =>
    api<ListResponse<NavigationNode>>(
      `/api/knowledge-graph/navigation/${nodeId}/children`
    ),
  ...
  create: (data: {
    ...
    api<{ node: NavigationNode }>("/api/knowledge-graph/navigation", {
      method: "POST",
      body: JSON.stringify(data),
    }),
  ...
  migrate: (nodeId: string, targetDirId: string) =>
    api<{ node: NavigationNode }>(
      `/api/knowledge-graph/navigation/${nodeId}/migrate`,
      {
        method: "POST",
...
export const messagesApi = {
  list: (convId: string, options?: { limit?: number; offset?: number; tree?: boolean }) => {
    ...
    return api<ListResponse<Message>>(
      `/api/knowledge-graph/conversations/${convId}/messages${qs ? `?${qs}` : ""}`
    );
  },

  get: (msgId: string) =>
    api<{ message: Message }>(`/api/knowledge-graph/messages/${msgId}`),
  ...
  create: (data: {
    ...
    api<{ message: Message }>("/api/knowledge-graph/messages", {
      method: "POST",
      body: JSON.stringify(data),
    }),
  ...
  delete: (msgId: string) =>
    api<{ ok: boolean }>(`/api/knowledge-graph/messages/${msgId}`, {
      method: "DELETE",
    }),

  addKnowledgeNode: (msgId: string, nodeId: string) =>
    api<{ ok: boolean }>(
      `/api/knowledge-graph/messages/${msgId}/knowledge-nodes/${nodeId}`,
      { method: "POST" }
    ),
};

// ═══════════════════════════════════════════
//  KnowledgeGraph Service (unified)
// ═══════════════════════════════════════════
