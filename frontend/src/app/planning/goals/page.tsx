        ...
        {goals.length === 0 ? (
          <Card>
            <div className="text-center py-12">
              <Target size={40} className="mx-auto mb-3 text-[var(--color-text-muted)]" />
              <div className="text-sm text-[var(--color-text-muted)]">还没有目标</div>
      ...
      <div className="flex items-start justify-between gap-3 flex-wrap">
        <div className="flex-1 min-w-[200px]">
          <div className="flex items-center gap-2 mb-1 flex-wrap">
            ...
            <span className={`text-xs px-1.5 py-0.5 ${STATUS_COLORS[goal.status] || "bg-zinc-100"}`}>
              {STATUS_LABELS[goal.status] || goal.status}
            </span>
          </div>
          {goal.description && (
            <div className="text-sm text-[var(--color-text-muted)] mb-2">{goal.description}</div>
          )}
          ...
          <div className="mt-3 w-full bg-[var(--color-card)] border border-[var(--color-border)] h-2">
            <div
          ...
          {editing ? (
            <>
              <select
                ...
                className="px-2 py-1 text-sm border border-[var(--color-border)] bg-[var(--color-bg)]"
              >
              ...
              <button
                ...
                className="inline-flex items-center gap-1 px-3 py-1.5 text-xs bg-[var(--color-accent)] text-white"
              >
                <CheckCircle2 size={12} /> 保存
              </button>
              <button
                onClick={onCancelEdit}
                className="px-3 py-1.5 text-xs border border-[var(--color-border)]"
              >
                取消
              </button>
            </>
            ...
            <button
              onClick={onEdit}
              className="inline-flex items-center gap-1 px-3 py-1.5 text-xs border border-[var(--color-border)] hover:bg-[var(--color-card)]"
            >
              <Edit3 size={12} /> 状态
            </button>
          )}
