...
export default function FlashCardReviewPage() {
  ...
  const startSession = useCallback(async () => {
    ...
    try {
      ...
      setError(e instanceof Error ? e.message : "启动复习失败");
    } finally {
      setLoading(false);
    }
  }, []);
  ...
  if (!sessionId) {
    return (
      <div className="container mx-auto p-6 max-w-2xl">
        ...
        <div className="border border bg-surface rounded-lg p-6">
          <h1 className="text-xl font-semibold mb-4">📖 开始复习会话</h1>
          <p className="text-muted mb-4">
            FSRS 调度器将根据每张卡片的稳定性、难度、遗忘速率计算最优复习时间。
          </p>
          <ul className="text-sm space-y-1 text-muted list-disc pl-5 mb-4">
            <li>按"空格"或"回车"显示答案</li>
            <li>显示答案后, 按 <kbd className="px-1.5 py-0.5 rounded border border text-xs">1</kbd> 困难 / <kbd className="px-1.5 py-0.5 rounded border border text-xs">2</kbd> 良好 / <kbd className="px-1.5 py-0.5 rounded border border text-xs">3</kbd> 简单</li>
          ...
          <button
            ...
            disabled={loading}
            className="w-full px-4 py-2 text-sm rounded bg-success text-white hover:bg-success disabled:opacity-50"
          >
            {loading ? "加载中..." : "▶ 开始复习"}
          </button>
        </div>
      ...
      <div className="container mx-auto p-6 max-w-2xl">
        <div className="border border bg-surface rounded-lg p-12 text-center">
          ...
          <div className="flex gap-2 justify-center">
            <button
              onClick={() => router.push("/flashcard")}
              className="px-4 py-2 text-sm rounded border border hover:bg-surface-hover"
            >
              返回列表
            </button>
            <button
    ...
    <div className="container mx-auto p-6 max-w-3xl">
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-2 text-xs">
          <span className="px-2 py-0.5 rounded bg-danger/10 text-danger">
            困难 {stats.difficult}
          </span>
        ...
        <button
          onClick={endSession}
          className="px-3 py-1 text-xs rounded border border hover:bg-surface-hover"
        >
          结束会话
        </button>
      </div>
      ...
      {/* 进度条 */}
      <div className="w-full bg-surface-hover rounded-full h-1.5 mb-6">
        <div
      ...
      {/* 卡片内容 */}
      <div className="border border bg-surface rounded-lg p-8 min-h-[300px] mb-4">
        <div className="flex items-center gap-1.5 mb-4 flex-wrap">
          ...
          {card.tags?.slice(0, 3).map((t) => (
            <span
              key={t}
              className="text-[10px] px-2 py-0.5 rounded bg-surface-hover text-muted"
            >
              #{t}
            </span>
          ))}
        ...
        {/* 分隔线 */}
        {showBack && <hr className="my-4 border-dashed border" />}

        {/* 反面 */}
        {showBack && (
          <div className="text-base leading-relaxed whitespace-pre-wrap text-foreground/90">
            {card.back_text || <span className="text-muted">(无答案)</span>}
          </div>
        )}

        {/* 上次复习结果 (FSRS 可观测) */}
        {lastResult && (
          <div className="mt-6 p-4 bg-success/5 border border-success/20 rounded space-y-2 text-sm">
            <div className="font-medium flex items-center gap-1 text-success">
              ✓ 已记录: {ASSESSMENT_LABELS[lastResult.self_assessment]}
            </div>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-xs">
              <div>
                <div className="text-muted">稳定性</div>
        ...
        {!lastResult && showBack && card.stability != null && (
          <div className="mt-6 p-3 bg-surface-hover rounded grid grid-cols-3 gap-3 text-xs">
      ...
      {!showBack ? (
        <button
          ...
          className="w-full px-4 py-3 text-sm rounded bg-info text-white hover:bg-info disabled:opacity-50"
          disabled={loading}
        >
          👁 显示答案 (空格)
        </button>
        ...
        <div className="grid grid-cols-3 gap-2">
          <button
            ...
            disabled={loading}
            className="px-4 py-3 text-sm rounded bg-danger text-white hover:bg-danger disabled:opacity-50"
          >
            ✕ 困难 (1)
          </button>
          <button
            ...
            disabled={loading}
            className="px-4 py-3 text-sm rounded bg-warning text-white hover:bg-warning disabled:opacity-50"
          >
            ✓ 良好 (2)
          </button>
          <button
            ...
            disabled={loading}
            className="px-4 py-3 text-sm rounded bg-success text-white hover:bg-success disabled:opacity-50"
          >
            → 简单 (3)
          </button>
        </div>
