...
export default function SecretarySettingsPage() {
  ...
  const loadAll = async () => {
    ...
    try {
      ...
      const [modRes, onbRes, prefRes, agentRes] = await Promise.all([
        ...
        authedFetch(`/api/secretary/preferences?user_id=${userId}`),
        authedFetch(`/api/secretary/agent/preferences?user_id=${userId}`),
      ]);
  ...
  if (loading) {
    return (
      <div className="max-w-2xl mx-auto p-6 text-center text-sm text-muted">
        加载中…
      </div>
    );
  }
    ...
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h1 className="text-lg font-semibold text">秘书设置</h1>
        </div>
        <button
          onClick={() => { resetPrefs(); savePreferences(); }}
          className="text-xs px-2 py-1 rounded border border text-muted hover:text transition-colors"
        >
          重置偏好
        </button>
      </div>
      ...
      {isColdStart && onboarding && (
        <div className="p-4 rounded-lg border border-accent/20 bg-accent/5 transition-transform">
          <div className="flex items-center gap-2 mb-3">
            <BookOpen size={16} className="text-info" />
            <span className="text-sm font-semibold text">{onboarding.message}</span>
          </div>
      ...
      {/* ── 模块管理（已有） ── */}
      <div className="space-y-2">
        <h2 className="text-sm font-semibold text flex items-center gap-1.5">
          <Bell size={14} />
          模块管理
          <span className="text-[10px] text-muted font-normal ml-1">— 自定义秘书的功能模块</span>
        </h2>
        <div className="space-y-2">
          {modules.map((mod) => (
            <div
              ...
              className={`p-3 rounded-lg border transition-colors ${
                mod.enabled ? "border" : "border-dashed border opacity-60"
              }`}
            >
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <div className="text-sm font-medium text">{mod.display_name}</div>
                    <div className="text-[11px] text-muted">{mod.description}</div>
                  </div>
                </div>
                <button
                  ...
                  className={`relative w-10 h-5 rounded-full transition-colors flex-shrink-0 ${
                    saving === mod.name ? "opacity-50" : mod.enabled ? "bg-accent" : "bg-text-muted/30"
                  }`}
                >
                  <div
              ...
              {mod.enabled && mod.stats && (
                <div className="mt-2 ml-8 flex gap-3 text-[10px] text-muted">
                  <span>运行 {mod.stats.total_runs} 次</span>
                  <span>提案 {mod.stats.total_proposals} 条</span>
                  {mod.stats.errors > 0 && <span className="text-error">错误 {mod.stats.errors} 次</span>}
                </div>
              )}
            </div>
          ))}
      ...
      <div className="space-y-2">
        <h2 className="text-sm font-semibold text flex items-center gap-1.5">
          <Bot size={14} />
          AI 秘书
          <span className="text-[10px] text-muted font-normal ml-1">— 控制悬浮球助手的行为</span>
        </h2>
        <div className="space-y-3">
          {/* 确认模式 */}
          <div className="p-3 rounded-lg border border">
            <div className="text-xs font-medium text mb-2">跳转确认模式</div>
                ...
                <button
                  ...
                  className={`flex-1 p-2 rounded-lg text-xs border text-left transition-colors ${
                    agentPrefs.confirm_mode === mode
                      ...
                      : "border text-muted hover:border-text-muted"
                  }`}
                >
          ...
          <div className="p-3 rounded-lg border border">
            <div className="flex items-center justify-between mb-2">
              <div className="text-xs font-medium text">自动跳转置信度阈值</div>
              <span className="text-xs font-semibold text-accent">{agentPrefs.auto_jump_threshold}</span>
            </div>
            <input
              type="range"
              min={0.5}
      ...
      <div className="space-y-2">
        <h2 className="text-sm font-semibold text flex items-center gap-1.5">
          <Bell size={14} />
          通知偏好
          <span className="text-[10px] text-muted font-normal ml-1">— 控制哪些通知显示</span>
        </h2>
        <div className="space-y-1.5">
          {ALL_SOURCES.map((source) => (
            <div key={source} className="flex items-center justify-between p-2.5 rounded-lg border border">
              <div className="flex items-center gap-2">
                <span className="text-base">{SOURCE_EMOJIS[source]}</span>
                <span className="text-xs font-medium text">{SOURCE_LABELS[source]}</span>
              </div>
              <button
                ...
                className={`relative w-9 h-4.5 rounded-full transition-colors flex-shrink-0 ${
                  prefs.sourceEnabled[source] ? "bg-accent" : "bg-text-muted/30"
                }`}
              >
                <div
      ...
      {/* ── 优先级阈值 ── */}
      <div className="p-3 rounded-lg border border">
        <h2 className="text-sm font-semibold text flex items-center gap-1.5 mb-3">
          <Bell size={14} />
          最低优先级
          <span className="text-[10px] text-muted font-normal ml-1">— 仅显示 ≥ 此优先级的通知</span>
        </h2>
        <div className="flex items-center gap-2">
          <input
            type="range"
          ...
          />
          <span className="text-xs text-muted w-4 text-center">{prefs.priorityThreshold}</span>
      ...
      <div className="p-3 rounded-lg border border">
        <h2 className="text-sm font-semibold text flex items-center gap-1.5 mb-3">
          <Moon size={14} />
          安静时段
          <span className="text-[10px] text-muted font-normal ml-1">— 在此期间不推送通知</span>
        </h2>
        <div className="flex items-center gap-3">
          <input
            ...
            className="px-2 py-1 text-xs rounded border border bg-surface text"
          />
      ...
      <div className="p-3 rounded-lg border border">
        <h2 className="text-sm font-semibold text flex items-center gap-1.5 mb-3">
          <Bell size={14} />
          每日推送上限
          <span className="text-[10px] text-muted font-normal ml-1">— 0 = 无限</span>
        </h2>
        <div className="flex items-center gap-2">
          <input
            type="range"
            ...
            className="flex-1 accent-accent"
          />
          <span className="text-sm font-semibold text-accent w-6 text-center">{prefs.dailyPushLimit}</span>
          <span className="text-[10px] text-muted">条/天</span>
        </div>
      </div>
      ...
      <div className="flex items-center gap-2">
        <button
          ...
          className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium bg-accent text-white rounded-md hover:opacity-90 transition-opacity"
        >
          <RefreshCw size={12} />保存到服务器
        </button>
        <span className="text-[10px] text-muted">本地偏好已自动保存</span>
      </div>
      ...
      <div className="space-y-2">
        ...
        <div className="flex gap-2">
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-2 text-xs font-medium rounded-lg border border text hover:bg-surface transition-colors"
          >
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-2 text-xs font-medium rounded-lg border border-error/30 text-error hover:bg-error/10 transition-colors"
          >
            <Trash2 size={14} />删除数据
          </button>
        </div>
