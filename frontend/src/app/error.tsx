...
export default function Error({
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[400px] p-8 text-center">
      ...
      <p className="text-muted dark:text-muted mb-4 max-w-md text-sm">
        {error.message || '发生了意外错误'}
      </p>
      {/* 点击按钮触发 reset，尝试重新加载页面 */}
      <button
        onClick={reset}
        className="px-4 py-2 bg-accent text-white rounded-md hover:bg-accent-hover active:scale-[0.97] transition-colors"
      >
        再试一次
      </button>
    </div>
