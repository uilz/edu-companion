...
import {
  CalendarDays, ChevronLeft, ChevronRight, Loader2, Flame, Target, Zap,
} from "lucide-react";
...
/** 学习日历热力图标签（嵌入 analytics 页面使用，也可独立路由访问） */
export default function CalendarTab() {
  ...
  for (let d = 1; d <= totalDays; d++) {
    const entry = days.find(dd => dd.day === d) || {
      ...
      day: d, total: 0, correct: 0, accuracy: null,
    };
    cells.push(entry);
  }
          ...
          {cells.map((cell, i) => {
            ...
            return (
              ...
              >
                ...
                {cell.total > 0 && <span className="text-[8px] opacity-70">{cell.total}</span>}
              </button>
            );
          })}
        </div>
