...
export default function StatsTab() {
  ...
  if (loading) {
    return <div className="flex items-center justify-center py-20"><Loader2 className="animate-spin text-[var(--color-accent)]" size={24} /></div>;
  }
  ...
  return (
    <div>
      <h1 className="text-2xl font-semibold text-[var(--color-text)] mb-6 tracking-tight">学习统计</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
        <div>
          <h2 className="text-base font-semibold text-[var(--color-text)] mb-3 flex items-center gap-2">
            <Calendar size={14} className="text-[var(--color-accent)]" />近 30 天练习趋势
          </h2>
        ...
        <div>
          <h2 className="text-base font-semibold text-[var(--color-text)] mb-3 flex items-center gap-2">
            <RotateCcw size={14} className="text-amber-500" />待复习
          </h2>
          {overview && overview.due_review_count > 0 ? (
            <div className="bg-amber-500/10 border border-amber-500/30 rounded-xl p-4 mb-4">
              <div className="flex items-center justify-between">
                <div><span className="text-2xl font-bold text-amber-500">{overview.due_review_count}</span><span className="text-sm text-amber-500/80 ml-1">题到期</span></div>
                <Zap size={20} className="text-amber-500" />
              </div>
              <p className="text-[11px] text-[var(--color-text-muted)] mt-1">今日已完成 {overview.today_questions} 题</p>
            </div>
          ) : (
            <div className="bg-green-500/10 border border-green-500/30 rounded-xl p-4 mb-4">
              <div className="flex items-center gap-2"><Target size={16} className="text-green-500" /><span className="text-sm text-green-600">所有题目已复习！</span></div>
              <p className="text-[11px] text-[var(--color-text-muted)] mt-1">今日已完成 {overview?.today_questions ?? 0} 题</p>
            </div>
          )}

          <h2 className="text-base font-semibold text-[var(--color-text)] mt-4 mb-3 flex items-center gap-2">
            <Brain size={14} className="text-red-500" />薄弱知识点
          </h2>
          {weakSkills.length > 0 ? (
              ...
              {weakSkills.slice(0, 5).map(sk => (
                <div key={sk.skill_id} className="flex items-center justify-between p-2.5 rounded-lg bg-[var(--color-surface)] border border-[var(--color-border)]/60">
                  <div className="flex-1 min-w-0 mr-3">
                    ...
                    <p className="text-[10px] text-[var(--color-text-muted)]">{sk.attempts}次练习 · {sk.trend === "ascending" ? "📈" : sk.trend === "descending" ? "📉" : "➡️"}</p>
                  </div>
                  <div className="flex items-center gap-2">
                    <div className="w-16 h-1.5 bg-[var(--color-border)] rounded-full overflow-hidden">
                      <div className="h-full bg-red-500 rounded-full transition-all" style={{ width: `${sk.mastery * 100}%` }} />
                    </div>
                    <span className="text-[11px] text-[var(--color-text-muted)] w-8 text-right">{Math.round(sk.mastery * 100)}%</span>
                  </div>
                </div>
              ))}
            </div>
      ...
      <div className="mb-8">
        <h2 className="text-base font-semibold text-[var(--color-text)] mb-3 flex items-center gap-2">
          <BarChart3 size={14} className="text-[var(--color-accent)]" />最近练习
        </h2>
        {sessions.length > 0 ? (
          <div className="space-y-2">
            {sessions.map(s => {
              const scoreColor = s.score != null ? s.score >= 80 ? "text-green-500" : s.score >= 60 ? "text-yellow-500" : "text-red-500" : "text-[var(--color-text-muted)]";
              const modeLabel = s.mode === "adaptive" ? "自适应" : s.mode === "review" ? "复习" : s.mode === "challenge" ? "挑战" : s.mode;
              return (
                <div key={s.session_id} className="flex items-center justify-between p-3 rounded-lg bg-[var(--color-surface)] border border-[var(--color-border)]/60">
                  ...
                  <span className="text-[10px] text-[var(--color-text-muted)]">{s.created_at ? s.created_at.slice(0, 10) : ""}</span>
                </div>
              );
            })}
          </div>
