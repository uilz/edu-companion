  ...
  return (
    <>
      <button ref={btnRef}
        ...
        className="fixed w-[52px] h-[52px] rounded-full bg-accent text-white border-none cursor-pointer flex items-center justify-center shadow-lg z-50 hover:scale-105 transition-all duration-300 overflow-hidden"
        style={{
      ...
      {open && (
        <div className="fixed w-[380px] max-h-[560px] bg-surface border border rounded-xl shadow-2xl z-50 flex flex-col overflow-hidden animate-in slide-in-from-bottom-4 duration-200"
          style={{
          ...
          <div className="flex items-center gap-2 px-4 py-3 border-b border flex-shrink-0">
            ...
            {isNodeMode && dialogState?.boundNode && (
              <span className="text-[10px] px-2 py-0.5 rounded-full bg-accent/10 text-accent truncate max-w-[100px]">
          ...
          <div ref={listRef} className="flex-1 overflow-y-auto px-4 py-3 space-y-3 min-h-0">
            {displayMessages.length === 0 && !chat.streaming && (
              <div className="flex flex-col items-center justify-center h-full text-center gap-2 py-6">
                <Bot size={24} className="text-accent opacity-40" />
          ...
          <div className="flex-shrink-0 px-4 py-3 border-t border bg-surface">
            <div className="flex items-center gap-2">
              <input value={input} onChange={e => setInput(e.target.value)}
                onKeyDown={e => e.key === "Enter" && !chat.streaming && send()}
                placeholder="输入消息…"
                disabled={chat.streaming}
                className="flex-1 px-3 py-2 text-[12px] border border rounded-lg bg-page-secondary text placeholder:text-muted focus:outline-none focus:border-accent transition-colors disabled:opacity-50" />
