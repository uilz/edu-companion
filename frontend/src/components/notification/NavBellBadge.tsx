...
export default function NavBellBadge({ page, onToggle }: NavBellBadgeProps) {
  ...
  return (
    <button
      ...
      onClick={onToggle}
      className="relative inline-flex items-center justify-center p-1 text-[var(--color-text-secondary)] hover:text-[var(--color-text-primary)] transition-colors"
    ...
    >
      ...
      {unreadCount > 0 && (
        <span
          ...
          className="absolute -top-0.5 -right-0.5 min-w-[18px] h-[18px] flex items-center justify-center rounded-full bg-[var(--color-error)] text-white text-[10px] font-semibold leading-none px-1"
        >
