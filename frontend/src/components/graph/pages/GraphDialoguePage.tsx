...
function LoadingSkeleton() {
  return (
    <div className="flex h-full min-h-[600px] gap-0 animate-pulse">
      ...
      <div className="flex-1 bg-surface m-4 rounded-lg" />
    </div>
  );
}
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full min-h-[500px] gap-4">
      <div className="w-20 h-20 rounded-full bg-surface flex items-center justify-center">
        <GitGraph size={36} className="text-muted opacity-40" />
      </div>
      <h3 className="text-base font-medium text">还没有学习分区</h3>
      ...
      <div className="flex items-center gap-3">
        <a href="/"
          className="inline-flex items-center gap-2 px-5 py-2.5 text-sm bg-accent text-white rounded-lg hover:opacity-90 active:scale-[0.97] transition-all">
...
function EmptyState({ onLoad, onGenerate }: { onLoad: () => void; onGenerate: () => Promise<boolean> }) {
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full min-h-[500px] gap-4">
      ...
      <h3 className="text-base font-medium text">该分区暂无知识树</h3>
      <p className="text-xs text-muted max-w-xs text-center leading-relaxed">
        让 AI 为你生成该学科的知识树结构，
        <br />之后你可以手动补充和编辑。
      </p>
      <div className="flex items-center gap-3">
        <button onClick={handleGenerate} disabled={generating}
          className="inline-flex items-center gap-2 px-4 py-2 text-sm bg-accent text-white rounded-lg hover:opacity-90 disabled:opacity-50 active:scale-[0.97] transition-all">
          {generating ? <Loader2 size={14} className="animate-spin" /> : <Sparkles size={14} />}
          AI 预生成知识树
        </button>
        <button onClick={onLoad}
          className="inline-flex items-center gap-2 px-4 py-2 text-sm border border text-secondary rounded-lg hover:bg-surface">
          <RefreshCw size={14} /> 刷新
        </button>
      </div>
      <div className="mt-2 pt-4 border-t border/50 w-64 text-center">
        <p className="text-[10px] text-muted mb-2">或者先与 AI 对话，知识树会随学习自动生成</p>
        <a href="/"
          className="inline-flex items-center gap-2 px-4 py-2 text-sm border border-accent/30 text-accent rounded-lg hover:bg-accent/5 transition-all">
          <MessageCircle size={14} />
          开始临时对话
        </a>
      </div>
    ...
    return (
      <div className="flex items-center justify-center h-full min-h-[400px]">
        <div className="text-center space-y-3">
          <p className="text-sm text-error">加载失败: {ctx.error}</p>
          <button onClick={ctx.loadGraph} className="inline-flex items-center gap-1 text-xs text-accent hover:underline">
...
function GraphDialogueLayout({ ctx }: { ctx: UseGraphDialogueReturn }) {
  ...
  const modeBtn = (mode: GraphMode, label: string, icon: React.ReactNode) => (
    <button onClick={() => setGraphMode(mode)}
      className={`flex items-center gap-1 px-2 py-1 rounded-md text-[11px] transition-all ${
        graphMode === mode ? "bg-accent text-white font-medium shadow-sm" : "text-muted hover:text"
      }`}>{icon}{label}</button>
  );
    ...
    <button onClick={() => ctx.setLeftTab(key as any)}
      className={`flex items-center gap-1.5 pb-2 text-xs font-medium border-b-2 transition-all ${
        ctx.leftTab === key
          ...
          : "text-muted border-transparent hover:text"
      }`}>
      {icon}{label}
      {badge != null && badge > 0 && (
        <span className="text-[9px] px-1.5 py-0.5 rounded-full bg-surface-hover">{badge}</span>
      )}
    </button>
  );

  const graphTopbar = (
    <div className="flex-shrink-0 flex items-center gap-2 px-3 py-2 border-b border bg-surface">
      {/* 模式切换 */}
      <div className="flex items-center gap-1 bg-page-secondary rounded-lg border border p-0.5">
      ...
      <div className="flex-1 max-w-[180px] relative">
        <input value={ctx.graphSearch} onChange={(e) => ctx.setGraphSearch(e.target.value)}
          placeholder="搜索节点…"
          className="w-full pl-7 pr-2 py-1 text-[11px] rounded-md border border bg-page-secondary text placeholder:text-muted focus:outline-none focus:border-accent transition-colors"
        />
        <Search size={12} className="absolute left-2 top-1/2 -translate-y-1/2 text-muted" />
        {ctx.graphSearch && (
          <button onClick={() => ctx.setGraphSearch("")}
            className="absolute right-1.5 top-1/2 -translate-y-1/2 text-muted hover:text"><X size={12} /></button>
        )}
      </div>
      {ctx.graphSearch && ctx.matchedNodeIds.length > 0 && (
        <span className="text-[10px] text-accent font-medium">{ctx.matchedNodeIds.length} 匹配</span>
      )}
      ...
      <button onClick={() => setAddNodeOpen(true)}
        className="flex items-center gap-1 px-2 py-1 text-[10px] rounded-md bg-accent/10 text-accent hover:bg-accent/20 transition-all">
        <Plus size={12} />添加节点
      </button>

      <button onClick={() => ctx.setGraphFullscreen(!ctx.graphFullscreen)}
        className="p-1.5 rounded-md text-muted hover:text hover:bg-surface-hover transition-all"
  ...
  const statsBar = stats.total > 0 && (
    <div className="flex-shrink-0 flex items-center gap-4 px-3 py-1.5 border-b border bg-page-secondary text-[10px] text-muted">
      <span>📊 <strong className="text">{stats.total}</strong> 节点</span>
      <span>✅ 已掌握 <strong className="text-success">{stats.mastered}</strong></span>
      <span>📖 学习中 <strong className="text-warning">{stats.learning}</strong></span>
    ...
    return (
      <div className="fixed inset-0 z-50 bg-page">
        <button onClick={() => ctx.setGraphFullscreen(false)}
          className="absolute top-3 right-3 z-10 p-2 rounded-lg bg-surface border border text-muted hover:text transition-colors shadow-lg"
  ...
  const leftPanelInner = (
    <>
      <div className="flex-shrink-0 border-b border px-4 pt-3 pb-0 flex items-center gap-4 overflow-x-auto">
      ...
      <div className="flex-1 overflow-y-auto" onMouseUp={ctx.handleTextSelect}>
        <div className="p-4">
          {ctx.selectedNode && (
            <div className="mb-4 p-3 rounded-lg bg-accent/5 border border-accent/20 animate-fadeIn">
              <div className="flex items-center gap-2">
                ...
                <span className="text-[10px] px-1.5 py-0.5 rounded bg-surface-hover text-muted ml-auto">{ctx.selectedNode.level}</span>
              </div>
              <div className="flex items-center gap-3 mt-2 text-[11px] text-muted">
                <span>掌握度: {Math.round(ctx.selectedNode.mastery * 100)}%</span>
                <button onClick={() => ctx.handleRequestExplain(ctx.selectedNode!.id)} className="text-accent hover:underline ml-auto">请求讲解 →</button>
              </div>
            </div>
          )}
          ...
          {ctx.leftTab === "dialogue" && (
            <div className="animate-fadeIn h-full">
              {ctx.cardLoading ? (
                <div className="flex items-center justify-center py-8"><Loader2 size={16} className="animate-spin text-muted" /></div>
              ) : ctx.selectedNode ? (
          ...
          {ctx.leftTab === "notes" && (
            <div className="space-y-3 animate-fadeIn">
              <p className="text-xs text-muted">所有高亮、自我解释、反思自动汇聚为个人笔记流</p>
              {ctx.relatedNotes.length === 0 ? (
                <div className="text-center py-6"><FileText size={24} className="mx-auto mb-2 text-muted opacity-30" /><p className="text-xs text-muted">选中文本后使用工具栏添加笔记</p></div>
              ) : (
                ctx.relatedNotes.map(note => (
                  <div key={note.id} className="p-3 rounded-lg bg-surface border border transition-all hover:border-hover">
                    ...
                    <p className="text-xs text leading-relaxed">{note.content}</p>
                  </div>
                ))
              )}
              {ctx.relatedNotes.length > 0 && (
                <button onClick={() => ctx.setAggregateOpen(true)} className="flex items-center gap-1 text-xs text-accent hover:underline mx-auto"><Sparkles size={12} /> AI整理笔记</button>
              )}
            </div>
          )}
          ...
          {ctx.leftTab === "resources" && (
            <div className="space-y-3 animate-fadeIn">
              ...
              {ctx.selectedNode ? (
                <>
                  <div className="p-4 rounded-lg bg-surface border border text-center">
                    <Sparkles size={24} className="mx-auto mb-2 text-accent" />
                    <p className="text-xs text-muted">视频讲解功能正在接入中</p>
                  </div>
                </>
              ) : (
                <div className="text-center py-6"><Sparkles size={24} className="mx-auto mb-2 text-muted opacity-30" /><p className="text-xs text-muted">点击图谱中的节点查看资源</p></div>
              )}
            </div>
          )}
      ...
      <div className="flex flex-col h-full min-h-[600px]">
        ...
        {mobilePanelOpen && (
          <>
            ...
            <div className="fixed bottom-0 left-0 right-0 z-30 bg-page border-t border rounded-t-xl max-h-[75vh] flex flex-col animate-slideUp shadow-2xl">
              <div className="flex flex-col overflow-hidden">
                {leftPanelInner}
              </div>

              {hasNode && (
                <div className="flex-shrink-0 border-t border p-4">
                  <NodeDetailPanel
                    node={ctx.selectedNode!}
                    partitionId={ctx.partitionId}
        ...
        {addNodeOpen && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/30">
            <div className="bg-surface border border rounded-xl p-5 w-80 space-y-3 shadow-xl">
              <div className="flex items-center justify-between">
                <h3 className="text-sm font-semibold text">添加知识节点</h3>
                <button onClick={() => setAddNodeOpen(false)}><X size={14} className="text-muted" /></button>
              </div>
              ...
              <div className="flex gap-2 justify-end">
                <button onClick={() => setAddNodeOpen(false)} className="px-3 py-1.5 text-xs border border rounded-lg text-muted">取消</button>
                <button onClick={handleAddNode} disabled={addNodeLoading || !newNodeLabel.trim()}
                  className="px-3 py-1.5 text-xs bg-accent text-white rounded-lg hover:opacity-90 disabled:opacity-50">
  ...
  return (
    <div className="flex h-full min-h-[600px] transition-all duration-200">
      {/* 左侧面板 */}
      <div className="flex flex-col overflow-hidden border-r border"
