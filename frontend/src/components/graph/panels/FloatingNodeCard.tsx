    ...
    <>
      ...
      <div
        ...
        className="fixed z-50 top-1/2 right-8 -translate-y-1/2 w-72 max-h-[80vh] overflow-y-auto
          bg-[var(--color-surface-elevated)] border border-[var(--color-border)] rounded-xl shadow-2xl
          animate-in slide-in-from-right-4 fade-in duration-200"
      >
        {/* Header */}
        <div className="flex items-center justify-between px-4 py-3 border-b border-[var(--color-border)] sticky top-0 bg-[var(--color-surface-elevated)] z-10">
          <div className="flex items-center gap-2 min-w-0">
            {parentNode && onNavigateToParent && (
              <button
                onClick={() => onNavigateToParent(parentNode)}
                className="flex items-center gap-0.5 text-[10px] text-[var(--color-text-muted)] hover:text-[var(--color-accent)] transition-colors shrink-0"
                title={`返回父节点: ${parentNode.label}`}
              >
                <ChevronRight size={12} className="rotate-180" />
              </button>
            )}
            <span className="text-xs font-medium text-[var(--color-text-muted)] truncate">节点详情</span>
          </div>
          <button onClick={onClose}
            className="p-1 rounded text-[var(--color-text-muted)] hover:text-[var(--color-text)] hover:bg-[var(--color-surface-hover)] shrink-0">
            <X size={14} />
          </button>
        </div>
        ...
        <div className="p-4 space-y-3">
          ...
          <div>
            <div className="flex items-center gap-2 mb-1.5">
              ...
              <h3 className="text-sm font-semibold text-[var(--color-text)] leading-tight">{node.label}</h3>
              <span className="text-[9px] px-1.5 py-0.5 rounded bg-[var(--color-surface-hover)] text-[var(--color-text-muted)] ml-auto shrink-0">{node.level}</span>
            </div>
            {node.description && (
              <p className="text-[11px] text-[var(--color-text-secondary)] leading-relaxed line-clamp-3">{node.description}</p>
            )}
