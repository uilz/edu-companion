...
export default function NodeDetailPanel({
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden border-l border bg-surface">
      {/* Header */}
      <div className="flex-shrink-0 flex items-center justify-between px-4 py-3 border-b border">
        <div className="flex items-center gap-2">
          ...
          {!parentNode && <span className="text-xs font-medium text-muted">节点详情</span>}
        </div>
        <button onClick={onClose}
          className="p-1 rounded text-muted hover:text hover:bg-surface-hover">
          <X size={14} />
        </button>
      </div>
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="p-4 space-y-4">
          {/* Error */}
          {error && (
            <div className="flex items-center gap-1.5 p-2 rounded bg-danger/10 text-danger text-[11px]">
          ...
          {editing ? (
            ...
            <div className="space-y-3">
              <input value={editLabel} onChange={e => setEditLabel(e.target.value)}
                ...
                className="w-full px-3 py-2 text-xs rounded-lg bg-page border border focus:outline-none focus:border-accent" />
              <textarea value={editDesc} onChange={e => setEditDesc(e.target.value)}
                placeholder="描述（可选）" rows={3}
                className="w-full px-3 py-2 text-xs rounded-lg bg-page border border focus:outline-none focus:border-accent resize-none" />
              <input value={editTags} onChange={e => setEditTags(e.target.value)}
                placeholder="标签（逗号分隔）"
                className="w-full px-3 py-2 text-xs rounded-lg bg-page border border focus:outline-none focus:border-accent" />
              <div className="flex items-center gap-2">
                <button onClick={handleSave} disabled={saving}
                  className="flex items-center gap-1 px-3 py-1.5 text-[11px] rounded-lg bg-accent text-white hover:opacity-90 disabled:opacity-50">
                  {saving ? <Loader2 size={11} className="animate-spin" /> : <Save size={11} />}
                  保存
                </button>
                <button onClick={() => setEditing(false)}
                  className="px-3 py-1.5 text-[11px] rounded-lg border border text-muted hover:border-hover">
                  取消
                </button>
              </div>
            </div>
            ...
            <>
              ...
              <div>
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <h3 className="text-sm font-semibold text">{node.label}</h3>
                  <span className="text-[9px] px-1.5 py-0.5 rounded bg-surface-hover text-muted ml-auto">{node.level}</span>
                </div>
                {node.description && (
                  <p className="text-[11px] text-secondary leading-relaxed">{node.description}</p>
                )}
                ...
                <div>
                  ...
                  <div className="flex items-center gap-1 mt-1 flex-wrap">
                    {node.conv_ids.slice(0, 5).map(cid => (
                      <span key={cid} className="text-[9px] px-1.5 py-0.5 rounded bg-surface-hover text-muted truncate max-w-[100px]">{cid.slice(0, 12)}…</span>
                    ))}
                  </div>
                </div>
              )}
              ...
              <div className="space-y-2">
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  <button onClick={handleDelete} disabled={deleting}
                    className="flex items-center gap-1 px-2.5 py-1.5 text-[10px] rounded-lg border border text-danger hover:border-danger transition-colors">
          ...
          <div className="pt-3 border-t border">
            <span className="text-[10px] font-medium text-muted">AI 扩充</span>
            <div className="flex items-center gap-2 mt-2">
              <input type="number" value={expandDepth} onChange={e => setExpandDepth(Math.max(1, Math.min(5, Number(e.target.value))))}
                min={1} max={5} className="w-14 px-2 py-1 text-[11px] rounded border border bg-page focus:outline-none focus:border-accent" />
              <span className="text-[10px] text-muted">层深度</span>
              <button onClick={handleAiExpand} disabled={expanding}
                className="flex items-center gap-1 px-3 py-1.5 text-[11px] rounded-lg bg-accent/10 text-accent hover:bg-accent/20 disabled:opacity-50 transition-colors">
            ...
            {chatOpen && (
              <div className="mt-2 space-y-2">
                <div className="max-h-[200px] overflow-y-auto space-y-2">
                  {chatResp && (
                    <div className="p-2.5 rounded-lg bg-accent/5 border border-accent/10 text-[11px] text leading-relaxed">
                      {chatResp}
                    </div>
                  )}
                </div>
                <div className="flex items-center gap-2">
                  <input value={chatMsg} onChange={e => setChatMsg(e.target.value)}
                    ...
                    className="flex-1 px-3 py-1.5 text-[11px] rounded-lg bg-page border border focus:outline-none focus:border-accent"
                    onKeyDown={e => e.key === "Enter" && handleAiChat()} />
                  <button onClick={handleAiChat} disabled={chatLoading || !chatMsg.trim()}
                    className="p-1.5 rounded-lg bg-accent text-white hover:opacity-90 disabled:opacity-50">
          ...
          {node.children && node.children.length > 0 && (
            <div className="pt-3 border-t border">
              <span className="text-[10px] font-medium text-muted">子节点 ({node.children.length})</span>
              <div className="mt-1 space-y-1">
                {node.children.map(cid => (
                  <div key={cid} className="flex items-center gap-1.5 px-2 py-1 rounded text-[10px] text-secondary bg-surface-hover">
