    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
      ...
      <Card title="🔥 知识掌握度" className="!p-5">
        {masteryBars.length === 0 ? (
          <p className="text-xs text-muted">暂无数据，答几道题就出来了</p>
        ) : (
            ...
            {masteryBars.map((mb) => {
              ...
              return (
                <div key={mb.skill_id}>
                  ...
                  <div className="w-full h-1.5 bg-surface">
                    <div
      ...
      <Card title="📊 错因分布" className="!p-5">
        {errorDistribution.length === 0 ? (
          <p className="text-xs text-muted">暂无错题，继续保持！</p>
        ) : (
          <div className="space-y-3">
        ...
        {errorDistribution.length > 0 && (
          <Link
            ...
            className="inline-block mt-4 text-[11px] text-accent hover:underline"
          >
            查看全部错题 →
          </Link>
        )}
