...
export default function SummaryPanel({
  ...
  return (
    <div className="max-w-lg mx-auto px-4 py-12 text-center space-y-6">
      ...
      <div className="text-5xl font-bold text-[var(--color-accent)]">
        {score}
        <span className="text-base text-[var(--color-text-muted)]">分</span>
      </div>

      <div className="flex justify-center gap-8">
        <div className="text-center">
          <div className="text-xl font-semibold text-green-500">{correct}</div>
          <div className="text-xs text-[var(--color-text-muted)]">正确</div>
        </div>
        <div className="text-center">
          <div className="text-xl font-semibold text-red-500">{wrong}</div>
          <div className="text-xs text-[var(--color-text-muted)]">错误</div>
        </div>
        <div className="text-center">
          <div className="text-xl font-semibold text-[var(--color-text-muted)]">{total}</div>
          <div className="text-xs text-[var(--color-text-muted)]">总题数</div>
        </div>
      </div>
      ...
      <div className="flex justify-center gap-3 pt-2">
        ...
        {onViewBank && (
          <button onClick={onViewBank}
            className="px-5 py-2.5 rounded-xl border border-[var(--color-border)] text-sm text-[var(--color-text-muted)] hover:text-[var(--color-text)] transition-colors flex items-center gap-1.5">
