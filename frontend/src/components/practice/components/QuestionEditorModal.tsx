      ...
      <div className="w-full max-w-2xl max-h-[85vh] overflow-y-auto rounded-2xl bg-page border border shadow-xl p-6 space-y-4" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex gap-3">
          <select value={q.question_type} onChange={e => setQ({ ...q, question_type: e.target.value })}
            className="px-3 py-1.5 rounded-lg border border bg-page text-sm">
        ...
        <textarea value={q.stem} onChange={e => setQ({ ...q, stem: e.target.value })}
          ...
          className="w-full px-4 py-3 rounded-xl border border bg-page resize-none text-sm" />

        {/* 选项 */}
        {isOption && (
          <div className="space-y-2">
            <label className="text-xs text-muted">选项（点击右侧✓标记正确答案）</label>
            {q.options.map(opt => (
              <div key={opt.letter} className="flex items-center gap-2">
                <span className="shrink-0 w-7 text-center text-sm font-bold text-muted">{opt.letter}.</span>
                ...
                <button onClick={() => updateOpt(opt.letter, "is_correct", !opt.is_correct)}
                  className={`p-1.5 rounded-lg border transition-colors ${
                    opt.is_correct ? "border-success/40 bg-success/10 dark:bg-success/10 text-success" : "border text-muted"
                  }`}>
                  <Check size={14} />
                </button>
                <button onClick={() => removeOption(opt.letter)} className="p-1.5 rounded-lg border border text-muted hover:text-danger">
                  <X size={12} />
                </button>
              </div>
            ))}
            <button onClick={addOption}
              className="flex items-center gap-1 px-3 py-1.5 rounded-lg border border-dashed border text-xs text-muted hover:border-accent hover:text-accent">
              <Plus size={12} />添加选项
            </button>
          </div>
        )}
        ...
        {(q.question_type === "fill" || q.question_type === "free_form") && (
          <div>
            <label className="text-xs text-muted block mb-1">参考答案</label>
            <input value={q.answer[0] || ""} onChange={e => setQ({ ...q, answer: [e.target.value] })}
              placeholder="输入参考答案..."
              className="w-full px-4 py-2 rounded-lg border border bg-page text-sm" />
          </div>
        )}
        ...
        <textarea value={q.analysis} onChange={e => setQ({ ...q, analysis: e.target.value })}
          ...
          className="w-full px-4 py-2 rounded-xl border border bg-page resize-none text-sm" />

        {/* 保存 */}
        <div className="flex justify-end gap-2 pt-2">
          <button onClick={onClose}
            className="px-4 py-2 rounded-lg border border text-sm text-muted hover:text">
            取消
          </button>
          <button onClick={() => onSave(q)} disabled={!q.stem.trim()}
            className="px-4 py-2 rounded-lg bg-accent text-white text-sm hover:opacity-90 disabled:opacity-30 flex items-center gap-1.5">
            <Save size={14} />保存题目
          </button>
        </div>
      </div>
