...
§1  设计哲学
§2  系统架构
§3  核心模块（MDKS/ZPD/诊断/苏格拉底/多模态）
§4  数据模型
§5  API端点
§6  前端页面
§7  对话×练习完整联动 ← 重写（8个子模块）
§8  刻意练习与引导发现
§9  用户上传资料索引
§10 游戏化设计
§13 模块联动闭环（7个修复）
...
**决策**：所有练习最终都应从对话branch生成。"自由练习"是一个隐含对话。

**当前实现**（MVP过渡方案）：
- 独立练习页创建session，mode="adaptive"
- 原因：对话系统树结构会话尚未实现

**目标架构**（对话系统完成后修改）：
```
用户 → 选分区/知识点 → 后台创建临时branch
  → 系统问"想练哪个知识点？" → 用户回复 → PracticeTool出题
  → 练习结果写回branch
  → 下次打开对话，AI知道练习历史
```

**影响**：练习session_id → branch_id 绑定；ContextAwarePracticeTrigger 成为默认行为

---

## 16. 开发进度（2026-05-18 更新）

> **参考 PROGRESS.md 获取全系统总览**
...
| BKT引擎 | 4维MDKS+提示打折+解释评分+**持久化(load_or_create/save_state)** |
| 练习API | 19个端点(题目/会话/答题/提示/错题/统计/上下文触发/内联/回顾/推荐/行为分析) |
| LLM出题 | 7学科知识点模板+DeepSeek生成 |
| ZPD调度 | 甜蜜点计算+疲劳感知+SM-2间隔重复 |
| 前端练习页 | 创建→答题→提示→反馈完整流 |
| 前端错题本 | 状态筛选+标记掌握+材料引用 |
| 前端学情分析 | 6面板仪表板（概览/趋势/掌握/错因/热力/建议）+ 习惯养成Tab |
| 行为分析 | 最佳时段+streak+规律性+疲劳曲线+建议生成 |
| 习惯养成 | 每日目标+TinyHabits+番茄钟+SVG进度环 |
| 对话系统 | 树结构会话+多模态ContentBlock+智能分区+分支管理 |
| 对话×练习联动 | SharedKnowledgeState+内联练习+上下文选题+结果回写+LLM注入 |
| 资料索引(RAG) | PDF/Word/PPT/MD解析+Granite Embedding+pgvector语义搜索+资料出题 |
| 媒体搜索 | B站/百度/Bing/小红书搜索URL生成+练习错误自动推荐 |

### 待实现

| 优先级 | 模块 | 说明 |
|--------|------|------|
| P1 | PostgreSQL迁移 | 内存dict → 持久数据库，JSONB存多模态 |
| P1 | 前置知识点卡控(§14.2) | PREREQUISITES配置+难度调整 |
| P1 | 题目质量监控(§14.1) | 自动淘汰太简单/有歧义的题 |
| P2 | 知识图谱可视化 | 力导向图+前置关系可视化 |
| P2 | 对话→BKT知识更新 | update_from_conversation(§3.3) |
| P2 | 游戏化设计(§10) | 基于掌握的成就系统 |
| P3 | 自适应学习计划 | 基于ZPD+MDKS的学习路径生成 |
| P3 | 智能创造扩展 | 项目式学习+创造性练习 |
| P3 | 完整多模态输入 | 语音(STT)+手写识别+视频内嵌 |
