...
> **总代码量**: 后端 ~13,800 行 · 前端 ~6,100 行 · 文档 ~5,800 行  
> **API 端点**: 65 个 · **前端页面**: 10 个 · **核心服务**: 24 个
...
| # | 模块 | 完成度 | 已完成 | 待完成 |
|---|------|--------|--------|--------|
| 10 | **知识图谱** | 🟡 55% | 前端API实时图谱(38节点/45边)、6个API端点、前置卡控引擎、依赖图(YAML 40技能)、学习路径推荐 | BKT掌握度注入前端、力导向自动布局 |
| 11 | **多模态交互** | 🟡 40% | ContentBlock(text/image/audio/video)、文件上传 | 语音输入(STT)、手写识别、视频内嵌播放 |
