...
| BKT引擎 | 4维知识状态+提示打折+解释评分 |
| 练习API | 12个端点(题目/会话/答题/提示/错题/统计) |
...
| 习惯养成 | 每日目标+TinyHabits+番茄钟+进度环 |

### 待实现

| 优先级 | 模块 | 说明 | 依赖 |
|--------|------|------|------|
| P0 | 对话系统树结构会话 | partition/branch/node 后端 | — |
| P1 | 练习→对话联动(§7) | SharedKnowledgeState+内联练习+练习记忆 | P0 |
| P1 | 练习来源标签 | 做题时显示来源分区/分支 | P0 |
| P1 | 修改练习为对话触发 | 废弃独立session，练习即对话 | P0 |
| P2 | 用户资料索引(RAG) | PDF/图片上传→语义搜索→出题 | — |
| P2 | 行为分析+习惯养成 | ✅ 已实现：最佳时段+streak+规律性+番茄钟+TinyHabits | — |
| P2 | 前置知识点卡控 | PREREQUISITES配置+难度调整 | — |
| P2 | 题目质量监控 | 自动淘汰太简单/有歧义的题 | — |
