...
| 事件类 | event_type | 关键字段 | 说明 |
|--------|------------|----------|------|
| `CognitiveNodeLinked` | CognitiveNodeLinked | user_id, node_id, link_type, target_ref_type, target_ref_id, action | 节点与其他实体的链接变化（创建/删除/更新）|
| `CognitiveNodeMetadataChanged` | CognitiveNodeMetadataChanged | user_id, node_id, changed_fields | 节点元数据变化（描述/标签/层级等非 Belief 字段）|

> **重要**：旧 `CognitiveNodeUpdated` 已拆分为 `CognitiveNodeLinked` + `CognitiveNodeMetadataChanged`。
> 掌握度（Belief）变化由 cognitive engine 通过 `CognitiveEventRecord` 内部处理，不再走 `DomainEvent` 总线。
...
event_bus.subscribe("CognitiveNodeMetadataChanged", secretary_handler)
```
